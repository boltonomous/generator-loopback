...
var debug = require('debug')('loopback:generator:actions');
var readChunk = require('read-chunk');
var istextorbinary = require('istextorbinary');
var memfs = require('mem-fs-editor');
 ...
 */
    initWorkspaceForGenerator() {
      ...
      debug('the project directory is: %s', this.projectDir);
 ...
 */
    loadModelsForGenerator() {
      ...
      workspace.models.ModelDefinition.find(function(err, results) {
        ...
        debug('existing model names %s', this.modelNames);
 ...
 */
    ...
    copy(source, dest) {
      ...
      if (istextorbinary.isBinarySync(undefined, headers)) {
        debug('fs copy %s %s', source, this.destinationPath(dest));
        this.fs.copy(source, this.destinationPath(dest));
      } else {
        debug('template %s', source, dest);
    ...
    directory(source, destination) {
      ...
      // not used in `generator-loopback`
      // if (bulk) {
      //   cp = this.bulkCopy;
      // }
 ...
 */
    ...
    template(source, dest, data, options) {
      ...
      debug('template copy %s %s', this.templatePath(source), destPath);
      this.fs.copyTpl(
      ...
      //   options
      // );
      debug('assert file exists %s %s', destPath, fs.existsSync(destPath));
      debug('template copy finished!!!!!');
    };
  };
};
