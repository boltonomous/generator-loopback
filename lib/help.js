...
function argumentsBuild(generator) {
  var rows = generator._arguments.map(function(arg) {
    ...
    if (config) {
      config.desc = config.desc || config.description;
    }

    return [
      '',
      arg.name ? arg.name : '',
      (config && config.desc) ? '# ' + config.desc : '',
      (config && config.type) ? g.f('Type: ') + config.type.name : '',
      g.f('Required: ') + (config && config.required),
    ];
  });
